...
the COSMIC project, and select a set of mutations that fall within some genes of
interest.  From there on, we can pursue an analysis as described before.
...
#+BEGIN_SRC R
  ...
  vc = readVcfAsVRanges(vcf_path, "ncbi37", ScanVcfParam(info = "GENE"))

  vcs = vc[vc$GENE %in% genes]
#+END_SRC

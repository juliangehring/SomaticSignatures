...
\hypersetup{
  pdfkeywords={},
  pdfsubject={},
  pdfcreator={Emacs 24.1.1 (Org mode 8.2.5c)}}
\begin{document}
...
context \texttt{TAC}.  Thus, the somatic motif can be written as \texttt{TAC>TGC} or \texttt{TNC
...
Alternatively, we provide some functionality to extract the relevant information
from other sources of inputs.  At the moment, this covers the \emph{MuTect}
...
\cite{gaujoux_flexible_2010}.  The methods was used
...
In the following, the concept of somatic signatures and to steps of inferring
these from an actual biological data set is shown.  For this purpose, somatic
...
The \texttt{SomaticCancerAlterations} provides the somatic SNV calls for eight WES
studies, each investigating a different cancer type.  The metadata summarizes the
biological and experimental settings of each study.
...
somatic variants in terms of genomic location as well as reference and
alternative allele.  For more details about this class and how to construct it,
...
The results contains the decomposed matrices stored as a listed and can be
accessed using the standard R accessor functions.
